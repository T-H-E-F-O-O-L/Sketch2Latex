...
import { simpleMathExamples, simpleMathToLatex } from "./lib/simple-math";
...
const mathSymbolGroups = [
  { title: "Analyse", symbols: [["Fraction", "\\frac{a}{b}"], ["Racine", "\\sqrt{x}"], ["Intégrale", "\\int_a^b f(x)\\,dx"], ["Intégrale double", "\\iint_D f(x,y)\\,dA"], ["Somme", "\\sum_{k=0}^{n} u_k"], ["Produit", "\\prod_{k=1}^{n} a_k"], ["Limite", "\\lim_{x\\to a} f(x)"], ["Dérivée", "\\frac{d f}{d x}"], ["Dérivée partielle", "\\frac{\\partial f}{\\partial x}"], ["Gradient", "\\nabla f"], ["Laplacien", "\\Delta f"]] },
  { title: "Algèbre", symbols: [["Matrice", "\\begin{pmatrix}a&b\\\\c&d\\end{pmatrix}"], ["Déterminant", "\\det(A)"], ["Système", "\\begin{cases}ax+by=c\\\\dx+ey=f\\end{cases}"], ["Coefficient binomial", "\\binom{n}{k}"], ["Exponentielle complexe", "e^{i\\varphi}"], ["Conjugué", "\\overline{z}"], ["Partie réelle", "\\operatorname{Re}(z)"], ["Partie imaginaire", "\\operatorname{Im}(z)"]] },
  { title: "Vecteurs", symbols: [["Vecteur", "\\vec{u}"], ["Flèche longue", "\\overrightarrow{AB}"], ["Norme", "\\lVert\\vec{u}\\rVert"], ["Produit scalaire", "\\vec{u}\\cdot\\vec{v}"], ["Produit vectoriel", "\\vec{u}\\times\\vec{v}"], ["Base", "(\\vec e_x,\\vec e_y,\\vec e_z)"]] },
  { title: "Ensembles et logique", symbols: [["Réels", "\\mathbb{R}"], ["Complexes", "\\mathbb{C}"], ["Entiers", "\\mathbb{Z}"], ["Appartient", "\\in"], ["Inclus", "\\subset"], ["Pour tout", "\\forall"], ["Il existe", "\\exists"], ["Implique", "\\Rightarrow"], ["Équivalent", "\\Leftrightarrow"], ["Union", "\\cup"], ["Intersection", "\\cap"]] },
  { title: "Lettres grecques", symbols: [["α", "\\alpha"], ["β", "\\beta"], ["γ", "\\gamma"], ["δ", "\\delta"], ["θ", "\\theta"], ["λ", "\\lambda"], ["μ", "\\mu"], ["π", "\\pi"], ["ρ", "\\rho"], ["σ", "\\sigma"], ["φ", "\\varphi"], ["ω", "\\omega"]] },
] as const;
...
export default function Home() {
  ...
  const [simpleFormula, setSimpleFormula] = useState("integrale de 0 a plus infini de exp(-x^2) dx egale racine de pi sur 2");
  ...
  const simpleMathResult = useMemo(() => simpleMathToLatex(simpleFormula), [simpleFormula]);
  ...
  const updateSimpleFormula = (value: string) => { const result = simpleMathToLatex(value); setSimpleFormula(value); if (result.ok) setMathFormula(result.latex); };
  ...
  return <main className={`app-shell mode-${mode}`}>
    ...
    <section className="editor-layout">
      <aside className="left-panel">
        ...
        {panel === "math" && <div className="math-panel"><h2>Éditeur de formule</h2><p>Écrivez en langage simple ou modifiez directement le LaTeX généré. Tout fonctionne localement dans votre navigateur.</p><section className="simple-math-editor"><div className="simple-math-heading"><h3>Langage simple</h3><span>Local · sans API</span></div><textarea value={simpleFormula} onChange={(event) => updateSimpleFormula(event.target.value)} spellCheck="true" aria-label="Formule en langage simple" placeholder="Ex. integrale de 0 a infini de exp(-x^2) dx" /><p className={simpleMathResult.ok ? "simple-math-status ok" : "simple-math-status error"} aria-live="polite">{simpleMathResult.message}</p><div className="simple-math-examples" aria-label="Exemples de formules en langage simple">{simpleMathExamples.map((example) => <button key={example.label} onClick={() => updateSimpleFormula(example.input)}>{example.label}</button>)}</div></section><label className="math-editor-field"><span>Code LaTeX généré et modifiable</span><textarea className="math-input" value={mathFormula} onChange={(event) => setMathFormula(event.target.value)} spellCheck="false" aria-label="Formule mathématique LaTeX" /></label><span className="math-section-label">Aperçu</span><div className="math-live-preview" dangerouslySetInnerHTML={{ __html: mathPreview }} /><div className="math-actions"><button onClick={() => addMathEquation(false)}>Ajouter la formule</button><button onClick={() => addMathEquation(true)}>Ajouter comme démonstration</button><button onClick={() => { setSimpleFormula(""); setMathFormula(""); }}>Effacer</button></div>{mathSymbolGroups.map((group) => <details key={group.title} open={group.title === "Analyse"}><summary>{group.title}</summary><div className="math-token-grid">{group.symbols.map(([label, token]) => <button key={`${group.title}-${label}`} title={token} onClick={() => appendMathSymbol(token)}>{label}</button>)}</div></details>)}<h2>Formules de physique</h2><p>Choisissez une formule, puis adaptez les indices, hypothèses et notations avant de l’ajouter.</p>{physicsFormulaGroups.map((group) => <details key={group.title} open={group.title === "Mécanique"}><summary>{group.title}</summary><div className="physics-formula-list">{group.formulas.map(([label, formula]) => <button key={`${group.title}-${label}`} onClick={() => setMathFormula(formula)}><strong>{label}</strong><code>{formula}</code></button>)}</div></details>)}</div>}
