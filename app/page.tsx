...
import { CONCOURS_ARROW, CONCOURS_CONNECTOR_LABEL_OFFSET, CONCOURS_DASH, CONCOURS_GRAPH_GRID_PERCENT, CONCOURS_INK, CONCOURS_LIGHT_FILL, EXPORTED_SVG_STYLE, SVG_STROKE_PATTERNS, canvasUnitsToCentimeters, canvasUnitsToPoints } from "./lib/concours-style";
...
function concoursGraphPreview(object: CanvasObject, selected: boolean) {
  ...
  return <g><defs><clipPath id={clipId}><rect x={object.x} y={object.y} width={width} height={height} /></clipPath></defs>{object.graph?.showGrid === true && Array.from({ length: 9 }, (_, index) => <g key={index} opacity={CONCOURS_GRAPH_GRID_PERCENT / 100}><line x1={object.x + width * index / 8} y1={object.y} x2={object.x + width * index / 8} y2={object.y + height} stroke={color} /><line x1={object.x} y1={object.y + height * index / 8} x2={object.x + width} y2={object.y + height * index / 8} stroke={color} /></g>)}{paths.map((path, index) => <path key={`${object.id}-${index}`} d={path} fill="none" stroke={color} strokeWidth={strokeWidthFor(object)} strokeDasharray={GRAPH_CANVAS_DASHES[index % GRAPH_CANVAS_DASHES.length]} clipPath={`url(#${clipId})`} pointerEvents="stroke" />)}<line {...common} x1={object.x} y1={horizontalAxis} x2={object.x + width} y2={horizontalAxis} markerEnd="url(#arrowhead)" /><line {...common} x1={verticalAxis} y1={object.y + height} x2={verticalAxis} y2={object.y} markerEnd="url(#arrowhead)" /><text className="diagram-label" x={object.x + width - 4} y={horizontalAxis - 8} textAnchor="end" fontSize="14" fill={color}>{scientificLabelSpans(object.graph?.xLabel ?? "x")}</text><text className="diagram-label" x={verticalAxis + 9} y={object.y + 15} fontSize="14" fill={color}>{scientificLabelSpans(object.graph?.yLabel ?? "y")}</text></g>;
}
