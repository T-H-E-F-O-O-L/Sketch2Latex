...
function stampPreview(object: CanvasObject, selected: boolean) {
  ...
  if (object.kind === "lens") return <g {...common}><line x1={x} y1={cy} x2={x+w} y2={cy} strokeWidth="1.5" /><line x1={cx} y1={y+h*.1} x2={cx} y2={y+h*.9} markerStart="url(#arrowhead)" markerEnd="url(#arrowhead)" /><circle cx={cx} cy={cy} r="2.5" fill="#111" />{text(a("origin", "O"), cx + 9, cy + 16)}</g>;
  if (object.kind === "diverging-lens") return <g {...common}><line x1={x} y1={cy} x2={x+w} y2={cy} strokeWidth="1.5" /><line x1={cx} y1={y+h*.1} x2={cx} y2={y+h*.43} markerEnd="url(#arrowhead)" /><line x1={cx} y1={y+h*.9} x2={cx} y2={y+h*.57} markerEnd="url(#arrowhead)" /><circle cx={cx} cy={cy} r="2.5" fill="#111" />{text(a("origin", "O"), cx + 9, cy + 16)}</g>;
