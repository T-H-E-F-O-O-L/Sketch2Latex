...
type CompileDiagnostic = { message: string; line?: number; objectId?: string; log?: string };
...
export default function Home() {
  ...
  const [compileDiagnostic, setCompileDiagnostic] = useState<CompileDiagnostic>();
  ...
  useEffect(() => () => { if (compiledPdfUrl) URL.revokeObjectURL(compiledPdfUrl); }, [compiledPdfUrl]);
  useEffect(() => () => { if (canvasPreviewUrl) URL.revokeObjectURL(canvasPreviewUrl); }, [canvasPreviewUrl]);
  ...
  const compileLatex = async () => {
    setCompileStatus("compiling"); setCompileDiagnostic(undefined); setNotice("Compilation LaTeX en cours…");
    const source = latexSource.includes("\\documentclass") ? latexSource : `\\documentclass[tikz,border=5pt]{standalone}\n\\usepackage{tikz,circuitikz,pgfplots,amsmath,amssymb}\n\\pgfplotsset{compat=1.18}\n\\usetikzlibrary{arrows.meta,decorations.pathmorphing,calc}\n\\begin{document}\n${latexSource}\n\\end{document}`;
    try {
      const response = await fetch("/api/compile", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ source }) });
      if (!response.ok) { const diagnostic = await response.json() as CompileDiagnostic; setCompileDiagnostic(diagnostic); setCompileStatus("error"); setNotice(diagnostic.message || "La compilation a échoué."); return; }
      const pdfUrl = URL.createObjectURL(await response.blob()); setCompiledPdfUrl(pdfUrl);
      if (svgRef.current) { const svg = new XMLSerializer().serializeToString(cleanSvg(svgRef.current, settings.width, settings.height)); setCanvasPreviewUrl(URL.createObjectURL(new Blob([svg], { type: "image/svg+xml" }))); }
      setCompileStatus("success"); setNotice("Compilation réussie. Comparez le canevas et le PDF LaTeX ci-dessous.");
    } catch (error) { const message = error instanceof Error ? error.message : "Service de compilation indisponible."; setCompileDiagnostic({ message }); setCompileStatus("error"); setNotice(message); }
  };
  ...
  return <main className={`app-shell mode-${mode}`}>
    ...
    <section className="editor-layout">
      ...
      <aside className="right-panel">
        ...
        <section className="export-panel"><h2>Prévisualiser et exporter</h2><div className="export-buttons"><button onClick={compileLatex} disabled={compileStatus === "compiling"}>{compileStatus === "compiling" ? "Compilation…" : "Compiler le LaTeX"}</button><button onClick={exportSvg}>SVG vectoriel</button><button onClick={exportPdf}>PDF vectoriel</button><button onClick={copyLatex}>Copier LaTeX</button></div><p className={`round-trip ${roundTrip.ok ? "ok" : "warning"}`}>{roundTrip.ok ? "✓" : "⚠"} {roundTrip.message}</p>{compileDiagnostic && <div className="compile-error" role="alert"><strong>Erreur LaTeX{compileDiagnostic.line ? `, ligne ${compileDiagnostic.line}` : ""}</strong><p>{compileDiagnostic.message}</p>{compileDiagnostic.objectId && <button onClick={() => { setSelectedIds([compileDiagnostic.objectId!]); setNotice(`Objet ${compileDiagnostic.objectId} sélectionné depuis l’erreur de compilation.`); }}>Sélectionner l’objet concerné</button>}<details><summary>Journal de compilation</summary><pre>{compileDiagnostic.log}</pre></details></div>}{compiledPdfUrl && <div className="preview-comparison"><figure><figcaption>Canevas de l’éditeur</figcaption>{canvasPreviewUrl && <object data={canvasPreviewUrl} type="image/svg+xml" aria-label="Aperçu du canevas de l’éditeur" />}</figure><figure><figcaption>Résultat LaTeX compilé</figcaption><iframe src={compiledPdfUrl} title="PDF LaTeX compilé" /></figure></div>}<p className="latex-hint">La compilation réelle est effectuée par un service LaTeX en ligne : le code du schéma lui est transmis. La comparaison met en évidence les différences entre le canevas et le PDF final.</p><label><input type="checkbox" checked={snippetOnly} onChange={(event) => setSnippetOnly(event.target.checked)} /> Extrait TikZ seul</label>{mode === "advanced" && <><div className="latex-actions"><button onClick={applyLatexToCanvas} disabled={!latexDirty}>Appliquer au canevas</button><button onClick={() => setLatexDraft(undefined)} disabled={!latexDirty}>Annuler l’édition</button></div><textarea className="latex-editor" value={latexSource} onChange={(event) => setLatexDraft(event.target.value === latex ? undefined : event.target.value)} spellCheck="false" aria-label="Code LaTeX/TikZ éditable" /><p className="latex-hint">L’import reconnaît les lignes, formes, courbes, chemins et équations. Les commandes non prises en charge deviennent des blocs TikZ protégés, conservés sans perte.</p></>}</section>
      </aside>
    </section>
  </main>;
}
