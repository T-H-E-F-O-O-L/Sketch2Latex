...
function stampPreview(object: CanvasObject, selected: boolean) {
  ...
  if (object.kind === "lens") return <g {...common}><line x1={cx} y1={y+h*.1} x2={cx} y2={y+h*.9} /><path d={`M ${cx-w*.08} ${y+h*.18} L ${cx} ${y+h*.06} L ${cx+w*.08} ${y+h*.18} Z M ${cx-w*.08} ${y+h*.82} L ${cx} ${y+h*.94} L ${cx+w*.08} ${y+h*.82} Z`} fill={color} stroke={color} /></g>;
  if (object.kind === "diverging-lens") return <g {...common}><line x1={cx} y1={y+h*.1} x2={cx} y2={y+h*.9} /><path d={`M ${cx-w*.08} ${y+h*.06} L ${cx} ${y+h*.18} L ${cx+w*.08} ${y+h*.06} Z M ${cx-w*.08} ${y+h*.94} L ${cx} ${y+h*.82} L ${cx+w*.08} ${y+h*.94} Z`} fill={color} stroke={color} /></g>;
