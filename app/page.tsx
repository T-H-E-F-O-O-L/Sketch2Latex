...
function scientificScenePreview(scene: ScientificPrimitive[], object: CanvasObject, selected: boolean) {
  ...
  return <g>{scene.map((primitive, index) => {
    if (primitive.type === "line") return <line key={index} x1={primitive.x1} y1={primitive.y1} x2={primitive.x2} y2={primitive.y2} stroke={color} strokeWidth={primitive.strokeWidth ?? baseWidth} fill="none" strokeDasharray={primitive.dashed ? SVG_STROKE_PATTERNS.dashed : objectDash} markerEnd={primitive.arrowEnd ? "url(#arrowhead)" : undefined} />;
    ...
    if (primitive.type === "rect") return <rect key={index} x={primitive.x} y={primitive.y} width={primitive.width} height={primitive.height} stroke={color} strokeWidth={primitive.strokeWidth ?? baseWidth} strokeDasharray={objectDash} fill={fillFor(primitive.fill)} />;
    if (primitive.type === "polyline") {
      const points = primitive.points.map((point) => `${point.x},${point.y}`).join(" ");
      return primitive.closed ? <polygon key={index} points={points} stroke={color} strokeWidth={primitive.strokeWidth ?? baseWidth} strokeDasharray={objectDash} fill={fillFor(primitive.fill)} /> : <polyline key={index} points={points} stroke={color} strokeWidth={primitive.strokeWidth ?? baseWidth} strokeDasharray={objectDash} fill={fillFor(primitive.fill)} />;
    }
    ...
    const text = <text className={primitive.math === false ? "diagram-text" : "diagram-label"} x={primitive.x} y={primitive.y} textAnchor={primitive.anchor ?? "middle"} fontSize={primitive.fontSize ?? 14} fill={color}>{primitive.math === false ? primitive.value : scientificLabelSpans(primitive.latex ?? primitive.value)}</text>;
